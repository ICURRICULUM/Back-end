...
public class 컴공22SData {
    ...
    public 컴공22SData(MemberRepository memberRepository,
    ...
    public List<Take> getTakesData(Member member) {
        return Arrays.asList(
            ...
            Take.builder().category(Category.교양선택).majorType(주전공)
                ...
                .member(member).build(), // 첨단 바이오테크놀로지의 이해
            Take.builder().category(Category.교양필수).majorType(주전공)
            ...
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("GEB1107").get())
                .member(member).build(), // 의사소통 영어
            Take.builder().category(Category.교양선택).majorType(주전공)
