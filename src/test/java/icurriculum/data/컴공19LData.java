...
public class 컴공19LData {
    ...
    public 컴공19LData(MemberRepository memberRepository,
    ...
    public List<Take> getTakesData(Member member) {
        return Arrays.asList(
            ...
            Take.builder().category(Category.핵심교양2).majorType(주전공)
                .course(courseRepository.findByCode("GEC2001").get()).member(member).build(),
            // 부와 빈곤의 글로벌 지도
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEG2033").get()).member(member).build(),
            // 대학기초영어
            Take.builder().category(Category.교양필수).majorType(주전공)
            ...
            // 인터넷 프로그래밍
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("GEB1107").get()).member(member).build(),
            // 의사소통 영어
            Take.builder().category(Category.교양필수).majorType(주전공)
            ...
            Take.builder().category(Category.핵심교양3).majorType(주전공)
                .course(courseRepository.findByCode("GED3002").get()).member(member).build(),
            // 영화로보는문학세계
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("GEE3029").get()).member(member).build(),
            // 고급대학영어
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEE6008").get()).member(member).build(), // 스키
            ...
            // 3학년 2학기
            Take.builder().category(Category.전공선택).majorType(주전공)
                .course(courseRepository.findByCode("CSE3210").get()).member(member).build(),
            Take.builder().category(Category.전공선택).majorType(주전공)
