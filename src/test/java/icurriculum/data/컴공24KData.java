...
public class 컴공24KData {

    private static final String MEMBER_NAME = "홍길동";
    private static final Integer JOIN_YEAR = 24;
    ...
    public 컴공24KData(MemberRepository memberRepository,
    ...
    public List<Course> getCoursesData() {
        return Arrays.asList(
            ...
            Course.builder().code("GED2016").name("그리스신화의 이해").credit(3).build(),
            Course.builder().code("GEE2015").name("기초수학").credit(3).build(),
            Course.builder().code("GEE6001").name("건강한 체중조절을 위한 맞춤영양").credit(3).build(),

            Course.builder().code("GEE7016").name("탄소 중립과 에너지의 흐름").credit(3).build()
        );
    }
    ...
    public List<Take> getTakesData(Member member) {
        return Arrays.asList(
            ...
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GED2016").get())
                .member(member).build(), // 그리스신화의 이해
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEE2015").get())
            ...
            Take.builder().category(Category.전공필수).majorType(주전공)
                .course(courseRepository.findByCode("CSE1312").get())
                .member(member).build(), // 이산구조
            Take.builder().category(Category.전공필수).majorType(주전공)
                .course(courseRepository.findByCode("CSE2101").get())
                .member(member).build(), // 논리회로
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("GEB1109").get())
                .member(member).build(), // 의사소통 영어: 고급
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEE2024").get())
                .member(member).build(), // 미디어와 콘텐츠
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEE6001").get())
                .member(member).build(), // 건강한 체중조절을 위한 맞춤영양
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("MTH1002").get())
                .member(member).build(), // 일반수학 2

            Take.builder().category(Category.핵심교양6).majorType(주전공)
                .course(courseRepository.findByCode("GED6009").get())
                .member(member).build(), // 임의추가: 인공지능과 소프트웨어를 활용한 탄소중립

            Take.builder().category(Category.SW_AI).majorType(주전공)
                .course(courseRepository.findByCode("GEE7016").get())
                .member(member).build() // 임의추가: 탄소 중립과 에너지의 흐름
        );
    }
    ...
    public Core getCoreJsonData() {
        return Core.builder()
            ...
            .areaDeterminedCodeMap(Map.of(Category.핵심교양6, Set.of("GED6009")))
            .build();
    }
    ...
    public MajorRequired getTestMajorRequiredData() {
        return MajorRequired.builder()
            .codeSet(
                Set.of("CSE1101", "CSE1103", "CSE1112", "CSE1312", "CSE2101", "CSE2112", "CSE4205")
            )
            .build();
    }
