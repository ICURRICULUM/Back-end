...
public class 컴공20KData {

    private static final String MEMBER_NAME = "홍길동";
    private static final Integer JOIN_YEAR = 20;
    ...
    public 컴공20KData(MemberRepository memberRepository,
    ...
    public List<Take> getTakesData(Member member) {
        return Arrays.asList(
            ...
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEB1143").get()).member(member).build(),
            // 미래사회와 소프트웨어-IT 계열
            Take.builder().category(Category.교양필수).majorType(주전공)
            ...
            // 2학년 1학기
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("ACE2104").get()).member(member).build(), // 통계학
            Take.builder().category(Category.전공필수).majorType(주전공)
                .course(courseRepository.findByCode("CSE1112").get()).member(member).build(),
            // 컴퓨터공학입문
            ...
            Take.builder().category(Category.핵심교양1).majorType(주전공)
                .course(courseRepository.findByCode("GED1005").get()).member(member).build(),
            // 인간-AI-기계
            Take.builder().category(Category.교양선택).majorType(주전공)
                .course(courseRepository.findByCode("GEE2024").get()).member(member).build(),
            // 미디어와 콘텐츠
            ...
            // 논리회로
            Take.builder().category(Category.전공선택).majorType(주전공)
                .course(courseRepository.findByCode("CSE2105").get()).member(member).build(),
            // 컴퓨터기반선형대수
            Take.builder().category(Category.전공선택).majorType(주전공)
            ...
            // 시스템 프로그래밍
            Take.builder().category(Category.교양필수).majorType(주전공)
                .course(courseRepository.findByCode("GEE3027").get()).member(member).build(),
            // 실용영어 L/S
            ...
            Take.builder().category(Category.핵심교양5).majorType(주전공)
                .course(courseRepository.findByCode("GED5001").get()).member(member).build(),
            // 환경과 인간
            Take.builder().category(Category.핵심교양5).majorType(주전공)
                .course(courseRepository.findByCode("GED5007").get()).member(member).build()
            // 첨단 바이오테크놀로지의 이해
        );
    }
