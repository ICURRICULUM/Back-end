...
public abstract class ProcessorResponse {
    ...
    public static class CoreDTO {
        ...
        public void update(Take take, Iterator<Take> iterator, boolean isAreaAlternative) {
            completedCredit += take.getEffectiveCourse().getCredit();
            uncompletedArea.remove(take.getCategory());
    ...
    public static class GeneralRequiredDTO {
        ...
        public void initGeneralRequiredResponse(List<Course> generalRequiredCourseList) {
            uncompletedCourseSet.addAll(generalRequiredCourseList);
            for (Course course : generalRequiredCourseList) {
                generalRequiredCourseMap.put(course.getCode(), course);
            }
        }

        public void update(Take take, Iterator<Take> iterator, final int joinYear) {
            ...
            completedCredit += take.getEffectiveCourse().getCredit();
            uncompletedCourseSet.remove(
                generalRequiredCourseMap.get(take.getEffectiveCourse().getCode()));
    ...
    public static class MajorRequiredDTO {
        ...
        public void initMajorRequiredResponse(List<Course> majorRequiredCourseList) {
            uncompletedCourseSet.addAll(majorRequiredCourseList);
        ...
        public void update(Take take, Iterator<Take> iterator) {
            ...
            completedCredit += take.getEffectiveCourse().getCredit();
            uncompletedCourseSet.remove(
                majorRequiredCourseMap.get(take.getEffectiveCourse().getCode()));
            iterator.remove();
        }
    ...
    public static class MajorSelectDTO {
        ...
        public void update(Take take, Iterator<Take> iterator) {
            if (take.getCategory() != Category.전공선택) {
                log.error("전공선택으로 인정된 과목이지만, category가 전공선택이 아닙니다: {}", take);
                throw new GeneralException(ErrorStatus.CATEGORY_IS_NOT_VALID, take);
            }
    ...
    @Getter
    @ToString
    public static class GeneralSelectDTO {

        private int totalCompletedCredit; // 총 이수학점
        private int totalRequiredCredit; // 총 필요 이수학점
        private boolean isClear; // 조건 충족 여부

        public void update(LinkedList<Take> takeList, CreditData creditData) {
            totalCompletedCredit += TakeUtils.calculateTotalCredit(takeList);
            totalCompletedCredit += creditData.completedCreditExceptGeneralSelect();

            totalRequiredCredit = creditData.totalNeedCredit();
        }

        public void checkIsClear() {
            isClear = totalCompletedCredit >= totalRequiredCredit;
        }
    }
